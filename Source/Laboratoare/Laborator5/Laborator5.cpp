...
float ortoAngle = 60;
float ortoleft = -5.0, ortoright = 5.0, ortobottom = -5.0, ortotop = 5.0, ortoNear = -0.1, ortoFar = 10.0;
float FOV;
...
void Laborator5::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		...
		if (window->KeyHold(GLFW_KEY_C)) {
			...
			camera->RotateThirdPerson_OZ(cameraSpeed * deltaTime);
			// TODO : translate the camera up
		}
		...
		if (window->KeyHold(GLFW_KEY_1)) {
			...
			ortoleft -= deltaTime * 5;
			ortoright += deltaTime * 5;
			projectionMatrix = glm::ortho(ortoleft, ortoright, ortobottom, ortotop, ortoNear, ortoFar);
			// TODO : translate the camera up
		}
		if (window->KeyHold(GLFW_KEY_2)) {

			ortobottom -= deltaTime * 5;
			ortotop += deltaTime * 5;
			projectionMatrix = glm::ortho(ortoleft, ortoright, ortobottom, ortotop, ortoNear, ortoFar);
			// TODO : translate the camera up
		}
		...
		if (window->KeyHold(GLFW_KEY_B)) {
			...
			ortoAngle += deltaTime * 10;
			projectionMatrix = glm::perspective(RADIANS(ortoAngle), window->props.aspectRatio, 0.01f, 200.f);
			// TODO : translate the camera up
		}
