#include "Laborator3.h"

#include <vector>
#include <iostream>

#include <math.h>
#include <Core/Engine.h>
#include "Transform2D.h"
#include "Object2D.h"

#define PI 3.14159265

using namespace std;

Laborator3::Laborator3()
{
}

Laborator3::~Laborator3()
{
}

void Laborator3::Init()
{
	glm::ivec2 resolution = window->GetResolution();
	auto camera = GetSceneCamera();
	camera->SetOrthographic(0, (float)resolution.x, 0, (float)resolution.y, 0.01f, 400);
	camera->SetPosition(glm::vec3(0, 0, 50));
	camera->SetRotation(glm::vec3(0, 0, 0));
	camera->Update();
	GetCameraInput()->SetActive(false);

	glm::vec3 corner = glm::vec3(0, 0, 0);
	float squareSide = 100;

	// compute coordinates of square center
	float cx = corner.x + squareSide / 2;
	float cy = corner.y + squareSide / 2;

	// initialize tx and ty (the translation steps)
	translateX = 0;
	translateY = 0;

	// initialize sx and sy (the scale factors)
	scaleX = 1;
	scaleY = 1;

	// initialize angularStep
	angularStep = 0;


	Mesh* square1 = Object2D::CreateSquare("square1", corner, squareSide, glm::vec3(1, 0, 0), true);
	AddMeshToList(square1);

	Mesh* square2 = Object2D::CreateSquare("square2", corner, squareSide, glm::vec3(0, 1, 0), true);
	AddMeshToList(square2);

	Mesh* square3 = Object2D::CreateSquare("square3", corner, squareSide, glm::vec3(0, 0, 1), true);
	AddMeshToList(square3);

	Mesh* square4 = Object2D::CreateSquare("square4", corner, squareSide, glm::vec3(0, 0, 1), true);
	AddMeshToList(square4);

	Mesh* square5 = Object2D::CreateSquare("square5", corner, squareSide, glm::vec3(0, 0, 1), true);
	AddMeshToList(square5);

	Mesh* square6 = Object2D::CreateSquare("square6", corner, squareSide, glm::vec3(0, 0, 1), true);
	AddMeshToList(square6);
}

void Laborator3::FrameStart()
{
	// clears the color buffer (using the previously set color) and depth buffer
	glClearColor(0, 0, 0, 1);
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

	glm::ivec2 resolution = window->GetResolution();
	// sets the screen area where to draw
	glViewport(0, 0, resolution.x, resolution.y);
}

float sq1X = 150;
float sq2sc = 0;
float sq3rot = 0;
int speed1 = 50;
int speed2 = 1;
int speed3 = 1;

float Laborator3::Slide(float x, float dt) {

	if (x > 250 || x < 50) {

		speed1 *= -1;
	}

	x += speed1 * dt;
	return x;
}

float Laborator3::Inflate(float x, float dt) {

	if (x > 2.5f || x < 0) {

		speed2 *= -1;
	}

	x += speed2 * dt;
	
	return x;
}

float Laborator3::Rotate(float x, float dt) {

	if (x >= 2*PI) {

		x = 0;
	}
	x += speed3 * dt;
	return x;
}

float angle = 0;

void Laborator3::Update(float deltaTimeSeconds)
{

	// TODO: update steps for translation, rotation, scale, in order to create animations
	
	sq1X = Slide(sq1X, deltaTimeSeconds);

	modelMatrix = glm::mat3(1);
	modelMatrix *= Transform2D::Translate(sq1X, 250);
	// TODO: create animations by multiplying current transform matrix with matrices from Transform 2D

	RenderMesh2D(meshes["square1"], shaders["VertexColor"], modelMatrix);

	sq2sc = Inflate(sq2sc, deltaTimeSeconds);

	modelMatrix = glm::mat3(1);
	modelMatrix *= Transform2D::Translate(350, 250);
	modelMatrix *= Transform2D::Translate(50, 50);
	modelMatrix *= Transform2D::Scale(sq2sc, sq2sc);
	modelMatrix *= Transform2D::Translate(-50, -50);
	//TODO create animations by multiplying current transform matrix with matrices from Transform 2D
	
	RenderMesh2D(meshes["square2"], shaders["VertexColor"], modelMatrix);

	sq3rot = Rotate(sq3rot, deltaTimeSeconds);
	cout << sq3rot << endl;

	modelMatrix = glm::mat3(1);
	modelMatrix *= Transform2D::Translate(750, 250);
	modelMatrix *= Transform2D::Translate(50, 50);
	modelMatrix *= Transform2D::Rotate(sq3rot);
	modelMatrix *= Transform2D::Translate(-50, -50);

	//TODO create animations by multiplying current transform matrix with matrices from Transform 2D
	//RenderMesh2D(meshes["square3"], shaders["VertexColor"], modelMatrix);

	int sunX = 950;
	int sunY = 250;

	modelMatrix = glm::mat3(1);
	modelMatrix *= Transform2D::Translate(sunX, sunY);
	modelMatrix *= Transform2D::Scale(0.5f, 0.5f);


	RenderMesh2D(meshes["square4"], shaders["VertexColor"], modelMatrix);

	angle += deltaTimeSeconds;

	int distance = 150;

	int earthX = sunX + sin(angle) * distance;
	int earthY = sunY + cos(angle) * distance;

	modelMatrix = glm::mat3(1);
	modelMatrix *= Transform2D::Translate(earthX, earthY);
	modelMatrix *= Transform2D::Scale(0.25f, 0.25f);
	modelMatrix *= Transform2D::Translate(25, 25);
	modelMatrix *= Transform2D::Rotate(angle);
	modelMatrix *= Transform2D::Translate(-25, -25);


	RenderMesh2D(meshes["square5"], shaders["VertexColor"], modelMatrix);

	distance = 75;

	int moonX = earthX + sin(-2 * angle) * distance;
	int moonY = earthY + cos(-2 * angle) * distance;

	modelMatrix = glm::mat3(1);
	modelMatrix *= Transform2D::Translate(moonX, moonY);
	modelMatrix *= Transform2D::Scale(0.125f, 0.125f);
	modelMatrix *= Transform2D::Translate(12.5f, 12.5f);
	modelMatrix *= Transform2D::Rotate(-2 * angle);
	modelMatrix *= Transform2D::Translate(-12.5f, -12.5f);

	RenderMesh2D(meshes["square6"], shaders["VertexColor"], modelMatrix);
}

void Laborator3::FrameEnd()
{

}

void Laborator3::OnInputUpdate(float deltaTime, int mods)
{
	
}

void Laborator3::OnKeyPress(int key, int mods)
{
	// add key press event
}

void Laborator3::OnKeyRelease(int key, int mods)
{
	// add key release event
}

void Laborator3::OnMouseMove(int mouseX, int mouseY, int deltaX, int deltaY)
{
	// add mouse move event
}

void Laborator3::OnMouseBtnPress(int mouseX, int mouseY, int button, int mods)
{
	// add mouse button press event
}

void Laborator3::OnMouseBtnRelease(int mouseX, int mouseY, int button, int mods)
{
	// add mouse button release event
}

void Laborator3::OnMouseScroll(int mouseX, int mouseY, int offsetX, int offsetY)
{
}

void Laborator3::OnWindowResize(int width, int height)
{
}
